...
const csvFilename = jsonFilename => `${jsonFilename.replace(/\..+$/, "")}.csv`
...
program
  ...
  .option(
    "-j, --json <path>",
    "path to JSON file containing words to translate"
  )
  .parse(process.argv)
    ...
    let translated = await Promise.all(
      words.map(async word => {
        try {
          ...
          console.log(
            `translated ${chalk.magenta(word)} to ${chalk.blue(res.text)}`
          )
